...
#ifndef ONLINE_JUDGE
#define WATCH(key, val) printf("Watched %s ->  %s \n", key, std::to_string(val).c_str());
...
#define WATCH_MAP(map)                                                                                                 \
    printf("Watching Map\n");                                                                                          \
    for (auto e: map)                                                                                                  \
        printf("%s -> %s\n", std::to_string(e.first).c_str(), std::to_string(e.second).c_str());                       \
    printf("\n");
#else
...
int get_min(std::pmr::vector<std::pmr::vector<int>> a, int index) {
    using namespace std;
    int i, temp;
    int minium = 10000;
    vector<int> minium_index;
    int size = a.size()]
    // TODO:将这个比较改作一个fuction?
    FOR(i, size) {
        // 比较第一个数字，如果存在更小则重置最小角标这个数组
        if (a[i][index] < minium) {
            minium = a[i][1];
            minium_index.clear();
            minium_index.push_back(i);
        }
        // 如果相等则在最小角标数组里pushback这个角标
        if (a[i][index] == minium) {
            minium_index.push_back(i);
        }
    }
}
// peak这道题。要求给了一个vector，其中满足严格递增后递减的就是一个peak，其中子数组也是一个。要求输出满足条件的peak。例如1,
// 2 ,3 ,2, 1就是4 个（5个？）
void solve() {
    using namespace std;
